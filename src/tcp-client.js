...
(function(exports) {
  ...
  TcpClient.prototype.sendAPIMessage = function(category, request, values, callback){
    var msg = {
      "category": category,
      "request": request,
      "values": values
    };
    this.sendMessage(JSON.stringify(msg), callback);
  };

  TcpClient.prototype.sendHeartbeat = function(callback){
    var msg = {category: "heartbeat"};
    this.sendMessage(JSON.stringify(msg), callback);
  };

  TcpClient.prototype.startTracker = function(){
    this.sendAPIMessage(CATEGORY.TRACKER, REQUEST.SET, {"push": true, "version": 1});
  };
